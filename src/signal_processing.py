...
class SignalProcessing:
    # defines for output of identify_call_type
    ...
    def identify_call_type(call_file):
        ...
        if (round(f[np.argmax(tone_fft_mag)]) == 400 and max(tone_fft_mag)>config.dialing_tone_thresh):
            ...
            return SignalProcessing.FIRST_SPEAKER_CUSTOMER
        else:
            # this means customer is calling to the call center, represntative speaks first
            return SignalProcessing.FIRST_SPEAKER_REPRESENTATIVE
    ...
    def extract_spectrograms_from_utterances(utterances_list):
        """ Get a list of utterances and extract spectrograms for each frame 
    ...
    def split_segment_to_frames(seg):
        ...
        """
        # Extrct spectrogram
        spect = np.transpose(SignalProcessing.extract_spectrograms_from_utterances([seg])[0])
